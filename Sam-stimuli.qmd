...
create_lollipop_chart <- function(data, 
  ...
  p <- ggplot(plot_data, aes(x = category, y = value)) +
    ...
    annotation_custom(
      ...
      ymin = yupper * 1.20, ymax = yupper * 1.20, xmin = 0.5, xmax = 0.5
    ) +
...
```{r}
#| label: recycling-stimuli
home_order <- c("Single", "Couple", "Family")
muted_yellow <- "#C4A052"

create_bar_chart(
  data = recycling_data |> filter(Measure == "PercentEnrolled"),
  category_col = HomeType,
  value_col = Value,
  title = "Recycling Program Enrollment by Household Type",
  y_label = "% enrolled",
  category_order = home_order,
  y_break_interval = 10,
  color = muted_yellow
)
ggsave("sam-stimuli-pngs/recycling_bar.png")

create_lollipop_chart(
  data = recycling_data |> filter(Measure == "KgRecyclingPerWeek"),
  category_col = HomeType,
  value_col = Value,
  title = "Weekly Recycling by Household Type",
  y_label = "Recycling per\nweek (Kg)",
  category_order = home_order,
  y_break_interval = 2,
  color = muted_yellow
)
ggsave("sam-stimuli-pngs/recycling_lollipop.png")

create_dot_plot(
  data = recycling_data |> filter(Measure == "ContaminationRate"),
  category_col = HomeType,
  value_col = Value,
  title = "Recycling Contamination Rate by Household Type",
  y_label = "% contaminated",
  category_order = home_order,
  y_break_interval = 2,
  color = muted_yellow
)
ggsave("sam-stimuli-pngs/recycling_dot.png")
...
# Avg. Cups of Coffee per Day by Age
p1 <- create_bar_chart(
...
ggsave("sam-stimuli-pngs/coffee_bar.png")

# Percent Who Drink Black Coffee by Age
p2 <- create_lollipop_chart(
  data = coffee_data |> filter(Measure == "PercentDrinkingBlack"),
  category_col = AgeGroup,
  value_col = Value,
  title = "Percent Who Drink Black Coffee by Age",
  y_label = "% Drinking\nBlack",
  category_order = age_order,
  y_break_interval = 10,
  color = "#4C6A92"
)
ggsave("sam-stimuli-pngs/coffee_lollipop.png")

# Avg. Monthly Coffee Spending by Age
p3 <- create_dot_plot(
...
ggsave("sam-stimuli-pngs/coffee_dot.png")
...
region_order <- c("Northeast", "Midwest", "South", "Mountain", "Pacific")
create_bar_chart(
...
create_lollipop_chart(
  ...
  category_col = Location,
  value_col = Value,
  title = "Percent of Population Who Strength Train by U.S. Region",
  y_label = "% Who\nStrength Train",
  category_order = region_order,
  y_break_interval = 5,
  color = "#5C8A72",
...
create_bar_chart(
  ...
  value_col = Value,
  title = "Average Minutes of Spotify Listening by Time of Day",
  y_label = "Duration\n(minutes)",
...
create_dot_plot(
  ...
  category_col = TimeOfDay,
  value_col = Value,
  title = "Average Audio Volume Level by Time of Day",
  y_label = "Volume\n(dB)",
  category_order = time_order,
  y_break_interval = 10,
  color = "#8A6F9E",
...
create_bar_chart(
  ...
  value_col = Value,
  title = "Average Hours Spent Outside per Week by Temperature",
  y_label = "Hours per\nWeek",
...
create_lollipop_chart(
  data = outdoors_data |> filter(Measure == "PercentEnjoyOutdoors"),
  category_col = Temperature,
  value_col = Value,
  title = "Percent of Population Who Enjoy Outdoor Activities by Temperature",
  y_label = "% of\nPopulation",
  category_order = temp_order,
  y_break_interval = 10,
  color = "#2A9D8F",
  output_file = "sam-stimuli-pngs/outdoors_enjoyment_lollipop.png"
)

create_dot_plot(
  data = outdoors_data |> filter(Measure == "MonthlyGearSpending"),
  category_col = Temperature,
  value_col = Value,
  title = "Average Monthly Spending on Outdoor Gear by Temperature",
  y_label = "Monthly\nDollars",
  category_order = temp_order,
  y_break_interval = 10,
  color = "#2A9D8F",
  output_file = "sam-stimuli-pngs/outdoors_spending_dot.png"
)
```
